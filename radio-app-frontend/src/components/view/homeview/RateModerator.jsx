...
export function RateModerator({ loginToken, goToLogin, setToast, setIsHostCardOpen, hostImageSrc, hostName}) {
  ...
  const [hostComment, setHostComment] = useState("");
    ...
    const handleSubmitHostRating = async () => {
        ...
        if ((hostRating != "up" && hostRating != "down")) {
          setToast({
            type: "error",
            text: "Daumen hoch oder Daumen runter?",
          });
          return;
        }
        if ((hostComment == "")) {
          setToast({
            type: "error",
            text: "Huch? sieht aus als hättest du noch nichts geschrieben.",
          });
          return;
        }
        try {
